...
TC_CM_005 - Verify successful deletion of a contact
    [Documentation]  FR2.4 Verify successful deletion of a contact
    [Tags]  ContactManagement  Destructive
    ...
    Wait Until Page Contains  Delete contact  10s

    Click Element  xpath=//a[contains(text(), 'Delete contact')]
    Sleep  1s

    Wait Until Element Is Visible  xpath=//button[contains(text(), 'Confirm')]  10s
    Click Button  xpath=//button[contains(text(), 'Confirm')]

    Wait Until Location Contains  /people  10s

    Wait Until Page Does Not Contain  ${SAMPLE_FIRST_NAME}  10s
