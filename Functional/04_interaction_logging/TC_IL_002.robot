...
TC_IL_002 - Verify recording date and details for an interaction
    [Documentation]  FR4.2 Verify recording date and details for an interaction
    [Tags]  InteractionLogging
    ...
    Click Element  css=a[cy-name="add-call-button"]
    Wait Until Element Is Visible  css=div[cy-name="log-call-form"]  timeout=10s

    Input Text  xpath=//textarea[@placeholder="Write down what was said"]  Followed up with Abebe about their case.

    Execute JavaScript    document.querySelector('div[cy-name="log-call-form"] input[type="text"]').value = '2025-05-15'

    Click Button  css=button[cy-name="save-call-button"]
    Wait Until Page Contains  Followed up with Abebe about their case.  timeout=10s
